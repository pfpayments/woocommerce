...
class WC_PostFinanceCheckout_Admin_Order_Completion {
	...
	public static function init(){
		...
		add_action('wp_ajax_woocommerce_postfinancecheckout_execute_completion', array(
			__CLASS__,
			'execute_completion' 
		));
		...
		if ($gateway instanceof WC_PostFinanceCheckout_Gateway) {
		    ...
		    if ($transaction_info->get_state() == \PostFinanceCheckout\Sdk\Model\TransactionState::AUTHORIZED) {
				...
				echo '<button type="button" class="button button-primary postfinancecheckout-completion-button action-postfinancecheckout-completion-execute" style="display:none">' .
						 __('Execute Completion', 'woo-postfinancecheckout') . '</button>';
				echo '<label for="completion_restock_not_completed_items" style="display:none">' .
						 __('Restock not completed items', 'woo-postfinancecheckout') . '</label>';
				...
				echo '<label for="refund_amount" style="display:none">' . __('Completion Amount', 'woo-postfinancecheckout') . '</label>';
			}
		}
	}
	...
	protected static function update_line_items($completion_job_id){
		...
		try {
		    $line_items = WC_PostFinanceCheckout_Service_Line_Item::instance()->get_items_from_backend($completion_job->get_items(), $completion_job->get_amount(), 
					WC_Order_Factory::get_order($completion_job->get_order_id()));
		    WC_PostFinanceCheckout_Service_Transaction::instance()->update_line_items($completion_job->get_space_id(), $completion_job->get_transaction_id(), 
					$line_items);
	...
	public static function update_completions(){
		...
		foreach ($to_process as $id) {
			try {
				...
				$message = sprintf(__('Error updating completion job with id %d: %s', 'woo-postfinancecheckout'), $id, $e->getMessage());
